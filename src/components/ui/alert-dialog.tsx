	...
	return (
		<AlertDialogPortal>
			...
			<AlertDialogPrimitive.Content
				...
				data-size={size}
				className={cn(
					'bg-surface-elevated data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] group/alert-dialog-content fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-2xl border p-6 shadow-xl duration-200 data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-lg',
					className
				)}
				{...props}
			/>
...
const AlertDialogMedia = ({ className, ...props }: React.ComponentProps<'div'>) => {
	return (
		<div
			...
			className={cn(
				"bg-muted mb-2 inline-flex size-16 items-center justify-center rounded-[var(--radius-lg)] sm:group-data-[size=default]/alert-dialog-content:row-span-2 *:[svg:not([class*='size-'])]:size-8",
				className
			)}
			{...props}
		/>
